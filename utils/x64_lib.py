...
xmm_pool = set([f'XMM{i}' for i in range(16)])
ymm_pool = set([f'YMM{i}' for i in range(16)])

r64_pool = {'RAX', 'RBX', 'RCX','RDX','RSP','RBP','RSI','RDI', 'R8', 'R9','R10','R11','R12','R13','R14','R15'}
...
def regToSize(reg, size):
   ...
   elif size == 64: return regTo64(reg)
   elif size == 128: return re.sub('^[XYZ]', 'X', reg)
   elif size == 256: return re.sub('^[XYZ]', 'Y', reg)
   elif size == 512: return re.sub('^[XYZ]', 'Z', reg)
