...
class AnchorText(object):
    ...
    def explain_instance(self, text, classifier_fn, predicate_type, threshold=0.9, perturbation_probability=0.5,
                          delta=0.1, tau=0.15, batch_size=64, onepass=False,
                          use_proba=False, beam_size=4, use_stoke = False,
                          **kwargs):
        if type(text) == bytes:
            text = text.decode()

        all_types = ['token', 'instruction', 'opcode', 'dependency', 'register', 'instruction_dependency', 'instruction_dependency_num_insts', 'opcode_dependency_num_insts']
        predicates = []
        ...
        for predicate in predicates:
            ...
            if words == []:  # empty list of words
                print('Explanation: <no predicates>')  # printing the fact that there are no predicates, so it is easy to parse its results file
                continue
            ...
            exp = anchor_base.AnchorBaseBeam.anchor_beam(
                ...
                coverage_samples=10000, min_samples_start=batch_size, **kwargs)  # changed number of coverage samples from 1 to 2000
