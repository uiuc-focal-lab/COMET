...
#         print 'lm', lm, 'qm', qm, kl_bernoulli(p, qm)
    ...
    def get_anchor_from_tuple(t, state):
        ...
        for f in state['t_order'][t]:
            ...
            # print('coverage data:')
            # print(state['t_coverage'][(2,)], state['t_coverage'][(7,)])
            # print('done')
        return anchor
    ...
    def anchor_beam(sample_fn, delta=0.05, epsilon=0.1, batch_size=10,
        ...
        anchor = {'feature': [], 'mean': [], 'precision': [],
                  'coverage': [], 'examples': [], 'all_precision': 0}
        my_samples, coverage_data, _ = sample_fn([], coverage_samples, compute_labels=False)
        # my_samples = [m[0] for m in my_samples]
        # df = pd.DataFrame({'samples': my_samples})
        # df.to_csv('scratch1.csv', index=False)
        raw_data, data, labels = sample_fn([], max(1, min_samples_start))
        # my_samples = [m[0] for m in raw_data]
        # df = pd.DataFrame({'samples': my_samples, 'labels': labels})
        # df.to_csv('scratch2.csv', index=False)
        # exit(0)
        mean = labels.mean()
        beta = np.log(1. / delta)
        ...
        while current_size <= max_anchor_size:
            ...
            for i, t in zip(chosen_tuples, best_of_size[current_size]):
                ...
                if mean >= desired_confidence and lb > desired_confidence - epsilon_stop:
                    if verbose:
                        print('Found eligible explanation ', t, 'Coverage:',
        ...
        if best_tuple == ():
            ...
            if verbose:
                print('Could not find an explanation, now doing best of each size')
            tuples = []
